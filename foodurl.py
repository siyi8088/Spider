...
class URL():
    ...
    def createTable(self,region):
        ...
        try:
            cursor.execute('create table foodurl_%s(url_id int not null auto_increment,area varchar(255),sub_area varchar(255),shop_num smallint,url varchar(255),primary key(url_id))'%region)
            cursor.close()
        ...
        if key:
            ...
            for item in items:
                if item.xpath('./dd/span'):
                    pageurls.append(item.xpath('./dt/a/@href'))
                else:
